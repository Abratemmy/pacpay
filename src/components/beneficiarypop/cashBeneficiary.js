...
import { CASH } from "../../pages/constants/actionTypes"

function CashBeneficiary({ addBeneficiary, onCashBeneficiariesChanged, isEdit }) {
    ...
    const onSubmit = (data, ev) => {
        ...
        localStorage.setItem('CashList', JSON.stringify(list))
        dispatch({ type: CASH, list })
        console.log("Benelist", list)
        if (isEdit) {
            onCashBeneficiariesChanged?.(list);
        } else {
            addBeneficiary();
        }
...
export default CashBeneficiary