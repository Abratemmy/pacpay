...
function MainREGISTER() {
    ...
    const handleSecondError = (targets) => {
        let errorsValue = {};
    ...
    const nextTwo = (ev) => {
        ev.preventDefault()
        let v = handleSecondError(registerData);
        if (v > 0) {
            console.log("error");
        }
        else {
    ...
    const handlePasswordError = (targets) => {
        let errorsValue = {};
        if (!targets.password) {
            errorsValue.password = "Password is required"
        }
    ...
    const handleSubmit = (ev) => {
        ev.preventDefault()
        let v = handlePasswordError(registerData)
        ...
        //submit form here if no error availble
            ...
            setApiError(state?.authReducer?.authData?.error?.response?.data.meta.error.messages[0])
                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        <div className={toggleState === 2 ? "Registercontent active-Registercontent" : "Registercontent"}>
                            <div className='middle'>
                                ...
                                <div className='form-input form-div' style={{ paddingTop: "30px" }}>
                                ...
                                <div style={{ padding: "10px 0px 40px 0px", cursor: "pointer" }}>
                                    <div onClick={nextTwo} className="nav-link" style={{ border: "0", width: "50%" }} >Proceed to Set Password</div>
                                </div>


                                <div className='last' style={{ bottom: "0px", paddingTop: "40px" }} >
                                    <div className='no-acc' >
                                        Already have an account? <NavLink to="/login" className="signup"><div>Login Here</div>
                                            <span><img src={brush} alt="" /></span></NavLink>
                                    </div>
                        ...
                        <div className={toggleState === 3 ? "Registercontent active-Registercontent" : "Registercontent"}>
                            <div className='middle'>
                                <div style={{ padding: "10px 0px 20px 0px" }}>
                                    <span onClick={() => setToggleState(2)} style={{ color: "var(--blueColor)", cursor: "pointer" }}>
                                        <MdArrowBackIosNew className="icon" />
                                        <MdArrowBackIosNew className="icon icon2" style={{ position: "relative", left: "-9px" }} />
                                    </span>
                                </div>
                                ...
                                <div className='form-input form-div' style={{ padding: "30px 0px 20px 0px" }}>
                                ...
                                <div style={{ padding: "30px 0px 40px 0px", cursor: "pointer" }}>
                                    <div className='ApiError' style={{ textAlign: "left" }}>{ApiError}</div>
                                    {loading === true ? (
                                        <button type="submit" className="nav-link" disabled style={{ border: "0", background: "var(--lightblue)", width: "50%" }} >
                                            Registering...
                                        </button>
                                    )
                                        : (
                                            <button type="submit" className="nav-link" style={{ border: "0", width: "50%" }} >Proceed</button>
                                        )}
                                </div>
